...
import { Moon, Sun, Zap, MessageCircle, HelpCircle, Trash2 } from 'lucide-react';

export default function Header({ onClearChat, onShowHelp, messageCount }) {
  ...
  return (
    <header className="relative text-center py-4 bg-white/90 dark:bg-slate-800/90 shadow-lg backdrop-blur-md border-b border-gray-200 dark:border-slate-700 transition-all duration-300">
      ...
      <button
        onClick={toggleTheme}
        className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 transition-all duration-300 transform hover:scale-110 active:scale-95"
      ...
      <div className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 flex items-center gap-2">
        ...
        <button
          onClick={onClearChat}
          className="p-1.5 rounded-lg bg-gray-100 dark:bg-slate-700 hover:bg-red-100 dark:hover:bg-red-900/50 transition-all text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400"
